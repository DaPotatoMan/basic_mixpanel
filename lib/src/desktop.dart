...
class MixpanelDesktop extends MixpanelBase<MixpanelAnalytics> {
  ...
  alias(alias, distinctId) => track(
    r'$create_alias',
    properties: {'alias': alias, 'distinct_id': distinctId},
  );
  ...
  @override
  registerSuperProperties(props) => use((sdk) => sdk.engage(operation: .$set, value: props));
  ...
  setPeopleProp(prop, to) => use(
    (sdk) => sdk.engage(operation: .$set, value: {prop: to.toString()}),
  );
